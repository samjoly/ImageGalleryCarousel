 ...
 * NOTES:
 * Summary:
 * This class represents an image loading request in the ImageLoader system, encapsulating details like URL, callbacks, priority, and cancellation. It's designed for object pooling with methods for initialization and reset, enabling efficient memory usage and dynamic updating of priorities and callbacks.
 *
 * Key Features:
 * - Encapsulation of image loading details and cancellation support.
 * - Object pooling ready with initialization and reset methods.
 * - Dynamic updating of priorities and callbacks.
 *
 */
...
namespace SJ.ImageGallery {
    ...
    /// <summary>
    ///     Represents an image load request.
    /// </summary>
    public class ImageLoadRequest {
        ...
        public void UpdateProgress(float val) {
            progress = val;
            // Update the state based on the new progress
            state = val >= 1.0f ? ImageLoadRequestState.COMPLETED : ImageLoadRequestState.IN_PROGRESS;
            // Invoke the progress callback with the updated progress
